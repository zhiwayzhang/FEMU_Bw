...
static uint16_t nvme_create_sq(FemuCtrl *n, NvmeCmd *cmd) {
  ...
  if (nvme_init_sq(sq, n, prp1, sqid, cqid, qsize + 1,
                   NVME_SQ_FLAGS_QPRIO(qflags), NVME_SQ_FLAGS_PC(qflags))) {
    g_free(sq);
    return NVME_INVALID_FIELD | NVME_DNR;
  }
...
static uint16_t nvme_identify_ctrl(FemuCtrl *n, NvmeCmd *cmd) {
  ...
  return dma_read_prp(n, (uint8_t *)&n->id_ctrl, sizeof(n->id_ctrl), prp1,
                      prp2);
}
...
static uint16_t nvme_util_log(FemuCtrl *n, NvmeCmd *cmd, NvmeCqe *cqe) {
  ...
  case NVME_TOTAL_UTIL:
    ...
    // printf("nand_util = %lf\ngc_nand_util=%lf\n", nand_util_d,
    // gc_nand_util_d); 1.2222 calculate utilization , **use ssd_utilization()**
    ...
    uint64_t gc_utilization =
        n->ssd->gc_read_count * (uint64_t)NAND_READ_LATENCY +
        n->ssd->gc_write_count * (uint64_t)NAND_PROG_LATENCY +
        n->ssd->gc_erase_count * (uint64_t)NAND_ERASE_LATENCY;

    n->ssd->nand_utilization =
        utilization * 1.0 / (n->ssd->sp.tt_luns * n->ssd->sampling_interval);
    n->ssd->gc_nand_utilization =
        gc_utilization * 1.0 / (n->ssd->sp.tt_luns * n->ssd->sampling_interval);
    // n->ssd->host_nand_utilization = n->ssd->nand_utilization -
    // n->ssd->gc_nand_utilization;
    int nand_util = n->ssd->nand_utilization * 10000;
    int gc_util = n->ssd->gc_nand_utilization * 10000;
    // printf("nand_util_hx = %d\ngc_nand_util_hx=%d\nresult=%d\n", nand_util,
    // gc_util, nand_util+gc_util);
...
static uint16_t nvme_get_feature(FemuCtrl *n, NvmeCmd *cmd, NvmeCqe *cqe) {
  ...
  case NVME_LBA_RANGE_TYPE:
    ...
    return dma_read_prp(n, (uint8_t *)rt,
                        MIN(sizeof(*rt), (dw11 & 0x3f) * sizeof(*rt)), prp1,
                        prp2);
  case NVME_NUMBER_OF_QUEUES:
    cqe->n.result =
        cpu_to_le32((n->num_io_queues - 1) | ((n->num_io_queues - 1) << 16));
    break;
...
static uint16_t nvme_error_log_info(FemuCtrl *n, NvmeCmd *cmd,
                                    uint32_t buf_len) {
  uint32_t trans_len;
  uint64_t prp1 = le64_to_cpu(cmd->dptr.prp1);
  uint64_t prp2 = le64_to_cpu(cmd->dptr.prp2);
...
static uint16_t nvme_smart_info(FemuCtrl *n, NvmeCmd *cmd, uint32_t buf_len) {
  ...
  smart.power_on_hours[0] =
      cpu_to_le64(((current_seconds - n->start_time) / 60) / 60);
...
void nvme_process_sq_admin(void *opaque) {
  ...
  while (!(nvme_sq_empty(sq))) {
    if (sq->phys_contig) {
      ...
      addr = nvme_discontig(sq->prp_list, sq->head, n->page_size, n->sqe_size);
    }
